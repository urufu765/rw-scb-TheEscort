...
namespace TheEscort
{
    ...
    {
        public static readonly SSOracleBehavior.SubBehavior.SubBehavID MeatEscort = new SSOracleBehavior.SubBehavior.SubBehavID("MeatEscort", true);

        public static readonly SSOracleBehavior.Action MeatEscort_init = new SSOracleBehavior.Action("MeatEscort_Init", true);
        public static readonly SSOracleBehavior.Action MeatEscort_marked = new SSOracleBehavior.Action("MeatEscort_WithMark", true);
        public static readonly SSOracleBehavior.Action MeatEscort_getout = new SSOracleBehavior.Action("MeatEscort_GetOut", true);
        public static readonly SSOracleBehavior.Action MeatEscort_angery = new SSOracleBehavior.Action("MeatEscort_Angery", true);

        public static readonly Conversation.ID Pebbles_Escort = new Conversation.ID("Pebbles_Escort", true);
        public static readonly Conversation.ID Pebbles_Escort_2 = new Conversation.ID("Pebbles_Escort_Two", true);
        public static readonly Conversation.ID Pebbles_Escort_Insult = new Conversation.ID("Pebbles_Escort_Insult", true);
        public static readonly Conversation.ID Pebbles_Escort_Angery = new Conversation.ID("Pebbles_Escort_Angery", true);

        public static readonly SSOracleBehavior.SubBehavior.SubBehavID MeetEscort = new SSOracleBehavior.SubBehavior.SubBehavID("MeetEscort", true);

        public static readonly SSOracleBehavior.Action MeetEscort_norm = new SSOracleBehavior.Action("MeetEscort_Normal", true);
        public static readonly SSOracleBehavior.Action MeetEscort_unmarked = new SSOracleBehavior.Action("MeetEscort_Unmarked", true);
        public static readonly SSOracleBehavior.Action MeetEscort_marked = new SSOracleBehavior.Action("MeetEscort_Marked", true);
        public static readonly SSOracleBehavior.Action MeetEscort_markedpost = new SSOracleBehavior.Action("MeetEscort_MarkedPost", true);        

        public static readonly Conversation.ID Moon_Escort = new Conversation.ID("Moon_Escort", true);
        public static readonly Conversation.ID Moon_Escort_Post = new Conversation.ID("Moon_Escort_Post", true);
        public static readonly Conversation.ID Moon_Escort_Reentry = new Conversation.ID("Moon_Escort_Reentry", true);
                ...
                if (base.action == MeatEscort_getout){
					...
					if (this.inActionCounter > 100)
					{
						if (base.player.room == base.oracle.room)
						{
							...
							{
								base.player.mainBodyChunk.vel += RWCustom.Custom.DirVec(base.player.mainBodyChunk.pos, base.oracle.room.MiddleOfTile(28, 32)) * 2f * (1f - base.oracle.room.gravity) * Mathf.InverseLerp(20f, 150f, (float)this.inActionCounter);
							}
							return;
						}
        ...
        private static void Moon_Updates_Escort(On.SSOracleBehavior.orig_Update orig, SSOracleBehavior self, bool eu)
        {
            if (self.oracle.room.game.StoryCharacter == EscortMe && self.action == SSOracleBehavior.Action.General_GiveMark){
                ...
                if ((self.inActionCounter > 30 && self.inActionCounter < 300) || self.oracle.ID == MoreSlugcats.MoreSlugcatsEnums.OracleID.DM){
                    ...
                    Vector2 yeetus = Vector2.ClampMagnitude(self.oracle.room.MiddleOfTile(24, 14) - self.player.mainBodyChunk.pos, 40f) / 40f * 2.8f * Mathf.InverseLerp(30f, 160f, (float)self.inActionCounter);
                    if (ModManager.CoopAvailable){
        ...
        private static void Escort_Meets_Moon(On.SSOracleBehavior.PebblesConversation.orig_AddEvents orig, SSOracleBehavior.PebblesConversation self)
        {
            ...
            if (DoAConverse.TryGet(self.owner.oracle.room.game, out bool val) && val){
                ...
                if (self.id == Moon_Escort_Post){
                    ...
                    if (self.owner.oracle.room.game.GetStorySession.saveState.miscWorldSaveData.SSaiConversationsHad > 0){  // Has met bitch boy
