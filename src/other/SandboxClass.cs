    ...
    public partial class Escort
    {
        ...
        public void ThundahParry(Player self)
        {
            ...
            if (this.parryCD == 0)
            {
                ...
                // Colors
                Color explosionColor = new(0.7f, 1f, 1f);
                ...
                for (int i = 0; i < self.room.physicalObjects.Length; i++)
                {
                    for (int j = 0; j < self.room.physicalObjects[i].Count; j++)
                    {
                        ...
                        bool hitCreature = !ModManager.CoopAvailable || RWCustom.Custom.rainWorld.options.friendlyFire || self.room.physicalObjects[i][j] is not Player p || p.isNPC;
        ...
        public bool Eshelp_ForgorWhatThisDid(Player self, Vector2 direction)
        {
